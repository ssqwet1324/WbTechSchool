...
type ServiceConfig struct {
	...
	PackageWithMigrations string        `json:"PACKAGE_WITH_MIGRATIONS"`
}
...
func New() *ServiceConfig {
	s := &ServiceConfig{
		...
		ConnMaxLifetime:       getEnvDuration("CONN_MAX_LIFETIME", 30*time.Second),
		JWTSecret:             getEnv("JWT_SECRET", "secret"),
		PackageWithMigrations: getEnv("PACKAGE_WITH_MIGRATIONS", "./migrations"),
	}

	zlog.Logger.Info().Any("cfg", s).Msg("config loaded successfully")
